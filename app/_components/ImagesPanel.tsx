...
const ImagePane = ({allowScale = false, allowVariate = false, onScale, onVariate}: {allowScale?: boolean, allowVariate?: boolean, onScale: any, onVariate: any}) => {
    return (
        <div className="flex flex-col justify-center items-center w-1/2 h-1/2 group">
            <div className="flex gap-10">
                {allowScale && (
                    <div onClick={onScale} className="w-[50px] h-[50px] cursor-pointer rounded-full bg-white flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <img src={'/icons/scale.svg'} className="w-[25px]"/>
                    </div>
                )}
                {allowVariate && (
                    <div onClick={onVariate} className="w-[50px] h-[50px] cursor-pointer rounded-full bg-white flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity">
